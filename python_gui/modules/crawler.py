...
Fixes: All page.evaluate() JS uses separate module-level string constants.
No nested quote escaping issues across any Python version.
"""
import asyncio
import hashlib
...
_JS_BODY_TEXT = r"() => { const remove = ['nav','footer','header','script','style','noscript']; const clone = document.body.cloneNode(true); remove.forEach(tag => { clone.querySelectorAll(tag).forEach(el => el.remove()); }); return clone.innerText.replace(/\s+/g, ' ').trim().slice(0, 3000); }"
_JS_SCHEMA = """
() => {
    const scripts = Array.from(document.querySelectorAll('script[type="application/ld+json"]'));
    return scripts.map(s => {
        try { const d = JSON.parse(s.textContent); return d['@type'] || ''; }
        catch(e) { return ''; }
    }).filter(Boolean);
}
"""
...
def _make_links_js(base_origin):
    return f"() => Array.from(document.querySelectorAll('a[href]')).map(a => a.href).filter(href => href.startsWith('{base_origin}'))"
